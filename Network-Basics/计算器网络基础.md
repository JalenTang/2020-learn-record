...
1. TCP 连接是一个全双工模式的连接，
2. 通信一方发起关闭报文后，只能表示乙方没有数据需要发送，但是依然可以接受另一方传输过来的数据；
...
2. 非对称加密 `Asymmetric cryptography`/ 公开密钥加密 `Public-key cryptography`
...
2. HTTPS 连接时需要一套**CA 数字证书**，该证书由可信任的 CA 机构签发，证书内附带 CA 机构的**公钥 Pub-Key**，与之对应的**私钥 Private-Key**则保存在服务端
...
1. `Expires`：**HTTP/1.0**的字段，弃置为服务器返回请求过期的时间，表示在该时间之前，本地缓存始终有效
...
3. `max-age=<seconds>`：缓存到期的最大周期（相对时间），超过该事件则任务过期
4. `s-maxage=<seconds>`：覆盖 max-age 或者 Expires 头
...
3. 其他重要的属性：
   ...
   - `SameSite`：为阻止`CSRF`（跨站请求伪造攻击），可取值为： 
     - `None`：自动携带，浏览器会在**同站请求和跨站请求**下继续发送 cookies 
     - `Strict`：浏览器将只发送**相同站点**请求的 cookie 
     - `Lax`：目前新版本浏览器的默认值，阻止一些跨站子请求发送 Cookie， 
...
- 语法：
  1. 发送方：`window.postMessage(message, targetOrigin)`
...
3. [MDN HTTP Headers 全解](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers)
4. [浏览器缓存](https://juejin.im/post/5c4528a6f265da611a4822cc)
5. [HTTP 和 HTTPS](https://juejin.im/post/5af557a3f265da0b9265a498)
